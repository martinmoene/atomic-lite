...
table = (
    ...
    ( 'CMakeLists.txt'
        , r'\W{1,4}VERSION\W+([0-9]+\.[0-9]+\.[0-9]+)\W*$'
        , '\tVERSION {major}.{minor}.{patch}' )

    , ( 'CMakeLists.txt'
        , r'set\W+atomic_lite_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( atomic_lite_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+atomic_lite_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( atomic_lite_version "{major}.{minor}" )\n' )
#
#    , ( 'script/install-xxx-pkg.py'
#        , r'\atomic_lite_version\s+=\s+"([0-9]+\.[0-9]+\.[0-9]+)"\s*$'
#        , 'atomic_lite_version = "{major}.{minor}.{patch}"\n' )
...
#        , 'version = "{major}.{minor}.{patch}"' )
    ...
    , ( 'include/nonstd/atomic.hpp'
        , r'\#define\s+atomic_lite_MAJOR\s+[0-9]+\s*$'
        , '#define atomic_lite_MAJOR  {major}' )

    , ( 'include/nonstd/atomic.hpp'
        , r'\#define\s+atomic_lite_MINOR\s+[0-9]+\s*$'
        , '#define atomic_lite_MINOR  {minor}' )

    , ( 'include/nonstd/atomic.hpp'
        , r'\#define\s+atomic_lite_PATCH\s+[0-9]+\s*$'
        , '#define atomic_lite_PATCH  {patch}\n' )
)
